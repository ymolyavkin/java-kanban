...
public class Main {
   ...
   // private static final HttpTaskManager httpTaskManager = (HttpTaskManager) Managers.getDefault();
    ...
    private static void updateTaskById() {
        ...
        // Ищем задачу по id
        var task = httpTaskManager.findTaskByIdOrNull(id, false);
        ...
        if (task == null || task instanceof Subtask) {
            ...
            if (task instanceof Task) {
                ...
                httpTaskManager.updateEpic((EpicTask) task, newTitleAndDescription);
                ...
                if (changeSubtasks == 1) {
                    ...
                    Subtask subtask = httpTaskManager.findSubtaskByIdOrNull(subtaskId, subtasks);
                    if (subtask != null) {
                        ...
                        if (updatedSubtask != null) {
                            httpTaskManager.addSubtaskToEpic(epicTask, updatedSubtask);
    ...
    private static void deleteTaskById() {
        ...
        String stringId = scanner.nextLine();
        int id = stringToInt(stringId);
        boolean taskIsFound = httpTaskManager.deleteTaskById(id);
    ...
    private static void deleteAllTasks() {
        boolean taskExists = httpTaskManager.deleteAllTasks();
    ...
    private static void getBrowsingHistory() {
        List<AbstractTask> historyFindTask = httpTaskManager.getHistory();
